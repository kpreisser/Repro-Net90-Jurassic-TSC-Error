// NOTE about ENUMS:
// Since we don't use const enums any more, there needs to be a runtime reference for them. Therefore,
// every time you change some enum in this file, you need to update the "tsGeneratedEnums.js" file by using
// the output of the TypeScript Compiler (e.g. on https://www.typescriptlang.org/play/) when specifying the
// enums (namespaces without "declare" keyword and enums with "export" keyword) as TypeScript input.
// (The reason we don't use const enums any more is because their value would be hard-coded in existing script code,
// so if we change the enum values, the scripts would fail until they are recompiled.)
// However, note that some generated enums might be modified manually which is indicated by comments,
// which means you should preserve these changes.
...
declare namespace codabix {
    ...
    class Type {
        ...
        constructor(
            ...
            length?: number | null,
            arrayDimensions?: ArrayLike<number | null> | null,
        ...
        readonly arrayDimensions: readonly (number | null)[] | null;
